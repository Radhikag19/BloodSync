...
public class ProfileActivity extends AppCompatActivity {
    ...
    private void navigateToHomeBasedOnUserType() {
        // First check SharedPreferences for cached user type for faster access
        SharedPreferences prefs = getSharedPreferences("user_prefs", MODE_PRIVATE);
        String cachedUserType = prefs.getString("user_type", null);

        if (cachedUserType != null) {
            // We have cached user type, use it for navigation
            navigateBasedOnUserType(cachedUserType);
        } else {
            // Check Firebase for user type
                ...
                userRef.child("userType").addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            // Cache user type for future use
                            ...
                            startActivity(new Intent(ProfileActivity.this, MainActivityC.class));
                            finish();
                        }
                    }
                    ...
                    public void onCancelled(@NonNull DatabaseError databaseError) {
                        Toast.makeText(ProfileActivity.this, 
                                "Error checking user type: " + databaseError.getMessage(),
                                Toast.LENGTH_SHORT).show();
                        startActivity(new Intent(ProfileActivity.this, MainActivityC.class));
                        finish();
                    }
    ...
    private void navigateBasedOnUserType(String userType) {
        ...
        if ("donor".equalsIgnoreCase(userType)) {
            // Donor goes to MainActivity
            intent = new Intent(ProfileActivity.this, MainActivity.class);
        } else if ("seeker".equalsIgnoreCase(userType)) {
            // Seeker goes to MainActivityC
            intent = new Intent(ProfileActivity.this, MainActivityC.class);
        } else {
